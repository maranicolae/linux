...
irqreturn_t kbd_interrupt_handler(int irq_no, void *dev_id)
{
	...
	int is_pressed = is_key_press(scancode);
	int ascii_val = 0;

	if (is_pressed == 0) {
		ascii_val = get_ascii(scancode);
...
static int kbd_init(void)
{
	...
	if (!request_region(0x61, 1, "io port1")) {
     return -ENODEV;
	}

	if (!request_region(0x65, 1, "io port2")) {
     return -ENODEV;
	}
